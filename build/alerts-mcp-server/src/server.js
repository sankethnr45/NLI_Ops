...
function buildCriticalAlertsInput(args) {
    return {
        ...(args.assetType === undefined ? {} : { assetType: args.assetType }),
        ...(args.alertType === undefined ? {} : { alertType: args.alertType }),
    };
}
function buildHighVibrationInput(args) {
    return {
        ...(args.minimumVibration === undefined ? {} : { minimumVibration: args.minimumVibration }),
        ...(args.assetType === undefined ? {} : { assetType: args.assetType }),
    };
}
server.registerTool("getCriticalAlerts", {
    title: "Get Critical Alerts",
    description: "Returns critical operational alerts from MongoDB. Supports optional assetType and alertType filters.",
    inputSchema: {
...
server.registerTool("getHighVibrationAssets", {
    title: "Get High Vibration Assets",
    description: "Returns sensor readings from MongoDB where vibration is above a threshold, optionally filtered by asset type.",
    inputSchema: {
