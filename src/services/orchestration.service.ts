...
export async function runOperationalChat(
  ...
  const { result: toolPlan, durationMs: planningDurationMs } = await measureAsync(() =>
    llm.generateToolPlan(normalizedMessage, operationalToolSchemas),
  );
  ...
  const { result: followUpPlan, durationMs: followUpDurationMs } = await measureAsync(() =>
    llm.generateFollowUpToolPlan(normalizedMessage, toolResults, operationalToolSchemas),
  );
  ...
  const { result: retrievedResources, durationMs: retrievalDurationMs } = await measureAsync(() =>
    retrieveResourcesForQuery(normalizedMessage),
  );
  ...
  if (Array.isArray(retrievedResources) && retrievedResources.length > 0) {
    logger.info("Injecting retrieved knowledge into LLM context", {
      category: "knowledge",
      requestId,
      resources: retrievedResources.map((r: any) => r.id),
      retrievalDurationMs,
    });

    // Synthesize a knowledge tool result so the existing LLM summary flow can consume it.
    toolResults.push({ name: "knowledge", result: retrievedResources } as ToolResult);
  }

  const { result: response, durationMs: summaryDurationMs } = await measureAsync(() =>
    llm.summarizeWithToolResults(normalizedMessage, toolResults),
  );
...
export async function* streamOperationalChat(
  message: string,
  options: OperationalChatOptions = {},
): AsyncIterable<OperationalChatStreamEvent> {
  const normalizedMessage = validateMessage(message);
  const llm = options.llm ?? getLlmService();
  const toolExecutor = options.toolExecutor ?? executeOperationalTool;
  const requestId = `chat_${Date.now()}_${Math.random().toString(16).slice(2)}`;
