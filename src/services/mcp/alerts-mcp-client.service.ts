...
export async function getAlertsMcpClient(): Promise<Client> {
  if (client) {
    return client;
  }
...
export function getDiscoveredAlertsMcpTools(): string[] {
  return discoveredTools;
}

export async function callAlertsMcpTool(name: AlertsMcpToolName, args: Record<string, unknown>) {
...
export async function closeAlertsMcpClient(): Promise<void> {
  await client?.close();
