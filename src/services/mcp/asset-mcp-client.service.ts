...
export async function getAssetMcpClient(): Promise<Client> {
  if (client) {
    return client;
  }
...
export async function callAssetMcpTool(name: AssetMcpToolName, args: Record<string, unknown>) {
...
export async function closeAssetMcpClient(): Promise<void> {
  await client?.close();
